...
class Party(
    ...
    # Agregando un nuevo campo de tipo selection al modelo Party (Terceros)
    # El campo permite elegir una fuente de donde llegué a contactarme
    ...
    @classmethod
    def get_access_rules(cls):
        rules = super().get_access_rules()
        rules.update({
            'write': [('user_id', 'in', [1])]  # Allow only specific users to write
        })
        return rules
